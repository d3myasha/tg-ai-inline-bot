...
class DembelState:
    ...
    def should_update(self, today: date | None = None) -> bool:
        if today is None:
            today = date.today()
        today_str = today.isoformat()
        return self.last_updated != today_str and self.current_days > 0

    def apply_update(self, today: date | None = None) -> int | None:
        """Decrement and save. Returns new count or None if at zero."""
        if today is None:
            today = date.today()
        old = self.current_days
        self.current_days = max(0, self.current_days - 1)
        self.last_updated = today.isoformat()
        self.save()
        logger.info("Dembel: %d → %d", old, self.current_days)
        return self.current_days if self.current_days > 0 else 0
...
class DembelService:
    ...
    async def start(self) -> None:
        ...
        # Background loop every 60 minutes
        self._task = asyncio.create_task(self._run_loop())
        logger.info("Dembel service started, %d days remaining", self._state.current_days)
